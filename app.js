...
import { initializeApp } from "https://www.gstatic.com/firebasejs/10.0.0/firebase-app.js";
  ...
  import { getFirestore, deleteDoc,  doc, setDoc, getDoc, updateDoc, collection, query, where, getDocs,  addDoc, serverTimestamp, onSnapshot, orderBy, increment } from "https://www.gstatic.com/firebasejs/10.0.0/firebase-firestore.js";
...
onAuthStateChanged(auth, (user) => {
    ...
    if (user && uid) {
        ...
        if (location.pathname !== '/profile.html' && location.pathname !== '/dashboard.html' && location.pathname !== "/chat.html" ) {
            location.href = "profile.html"
        } 
    }
    else {
    ...
    onSnapshot(collection(db, "todos"), (data) => {
        data.docChanges().forEach((todo) => {
            ...
            if (todo.type === 'removed') {
                ...
                <li id='${todo.doc.id}'><br>
                
                <input class='todo-input' type='text' value='${todo.doc.data().value}' disabled/><br>
...
const addTodo = async () => {
    try {
        ...
        var todo = document.getElementById("floatingTextarea2");
        var date = new Date()
        const docRef = await addDoc(collection(db, "todos"), {
            value: input1.value  + todo.value,
